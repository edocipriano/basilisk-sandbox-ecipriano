...
#define frhov(f)(f*rho1v[] + (1-f)*rho2v[])
...
static void momentum_refine (Point point, scalar u) {
  ...
  foreach_child()
    rhou += cm[]*rhov[]*u[];
  double du = u[] - rhou/((1 << dimension)*(cm[] + SEPS)*frhov(f[]));
  foreach_child()
    u[] += du;
}
...
static void momentum_restriction (Point point, scalar u)
{
  ...
  foreach_child()
    rhou += cm[]*frhov(f[])*u[];
  u[] = rhou/((1 << dimension)*(cm[] + SEPS)*frhov(f[]));
}
#endif // TREE
