...
char* gas_species[NGS] = {"NC10H22", "N2"};
char* liq_species[NLS] = {"NC10H22"};
...
double D0 = 0.4e-3, effective_radius0, d_over_d02;

int main (void) {
  kinfolder = "evaporation/n-decane-in-nitrogen";
...
event bcs (i = 0) {
  ...
  scalar C10molar = XGList[0];
  scalar N2molar = XGList[1];

  C10molar[top] = dirichlet (0.);
  C10molar[right] = dirichlet (0.);

  N2molar[top] = dirichlet (1.);
  N2molar[right] = dirichlet (1.);
...
event adapt (i++) {
  scalar C10 = YList[0];
  adapt_wavelet_leave_interface ({C10,T,u.x,u.y}, {f},
      (double[]){1.e-1,1.e0,1.e-3,1.e-3}, maxlevel, minlevel, 1);
      //(double[]){1.e-1,1.e-1,1.e-3,1.e-3}, maxlevel, minlevel, 1);
}
...
event movie (t += 0.05; t <= 6) {
  clear();
  box();
  view (ty = -0.5);
  draw_vof ("f");
  squares ("NC10H22", min = 0., max = 1., linear = true);
  mirror ({1.,0.}) {
    draw_vof ("f");
...
event stop (i += 10, last) {
  if (d_over_d02 < 0.05)
    return 1;
}

//event snapshots (i++,last) {
//  char name[80];
