...
BinTable * binning_build_table (scalar binid, scalar * targets, double eps) {
  double L = 1./eps;
...
BinTable * binning (scalar * targets, double eps) {
  ...
  BinTable * table = binning_build_table (binid, tnorm, eps);
...
event init (i = 0) {
  foreach() {
    ...
    T[] = radialprofile (r, 0.2, 0.8, 300., 800.);
  ...
  // 1. Create the bin table
  BinTable * table = binning (targets, eps);

  // 2. Integrate the bins
  foreach_bin (table) {
    foreach_bin_target (bin) {
      target0 = target;
      target += 100.*dt;
