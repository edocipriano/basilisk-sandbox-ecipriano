...
int main (void) {
  ...
  /**
  **Case 1**: Inverse is false, Interface is false, 0 Layers.
  */

  fraction (f, sq(0.2) - sq(x) - sq(y));
  mapregion (H, f, nl=0, inverse=false, interface=false);
  write_picture ("case1.png");
  fprintf (stderr, "case1 = %g\n", statsf(H).sum);

  /**
  **Case 2**: Inverse is true, Interface is false, 0 Layers.
  */

  fraction (f, sq(0.2) - sq(x) - sq(y));
  mapregion (H, f, nl=0, inverse=true, interface=false);
  write_picture ("case2.png");
  fprintf (stderr, "case2 = %g\n", statsf(H).sum);

  /**
  **Case 3**: Inverse is true, Interface is false, 1 Layers.
  */

  fraction (f, sq(0.2) - sq(x) - sq(y));
  mapregion (H, f, nl=1, inverse=true, interface=false);
  write_picture ("case3.png");
  fprintf (stderr, "case3 = %g\n", statsf(H).sum);

  /**
  **Case 4**: Inverse is false, Interface is true, 2 Layers.
  */

  fraction (f, sq(0.2) - sq(x) - sq(y));
  mapregion (H, f, nl=2, inverse=false, interface=true);
  write_picture ("case4.png");
  fprintf (stderr, "case4 = %g\n", statsf(H).sum);

  /**
  **Case 5**: Inverse is true, Interface is true, 2 Layers.
  */

  fraction (f, sq(0.2) - sq(x) - sq(y));
  mapregion (H, f, nl=2, inverse=true, interface=true);
